...
export const Message = memo(function Message({ message, onRetry, isStreaming }: Props) {
	...
	if (message.role === "user") {
		if (message.isAgent) {
			...
			return (
				<div className="my-4">
					<div className="bg-[#232428] rounded-md border border-[#2a2b2e] px-5 py-2.5 inline-flex items-center gap-2.5 font-mono text-[13px] text-[#999]">
		...
		return (
			<div className="flex flex-col items-end mb-6 group">
				<div className="max-w-[80%] bg-[#232428] text-[#e0e0e0] border border-[#2a2b2e] px-4 py-2.5 rounded-md text-[15px] leading-relaxed">
			...
			<div className="text-[#e0e0e0] text-[15px] leading-[1.7]">
				{segments.map((seg, i) =>
					seg.type === "code" ? (
						...
						<div key={i} className="prose prose-invert prose-base max-w-none prose-headings:text-[#e0e0e0] prose-p:my-1 prose-p:leading-[1.7] prose-p:text-[15px] prose-code:text-[#e0e0e0] prose-code:bg-[#2a2b2e] prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-[13px] prose-code:font-normal prose-pre:bg-[#111] prose-pre:border prose-pre:border-[#2a2b2e] prose-pre:rounded-md">
							<Markdown remarkPlugins={REMARK_PLUGINS}>{seg.text}</Markdown>
						</div>
					)
				)}
				{message.isStreaming && (
					<span className="inline-block w-[2px] h-5 bg-[#e0e0e0] animate-pulse ml-0.5" />
				)}
			</div>
